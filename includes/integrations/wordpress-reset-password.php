...
add_action( 'lostpassword_post', function ( $errors ) {
	...
	if ( gatecha_plugin_active( 'woocommerce' ) && isset( $_POST['woocommerce-lost-password-nonce'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Detecting which host form is being submitted by field presence only; no data is processed and the host handles its own nonce.
		return;
	}
